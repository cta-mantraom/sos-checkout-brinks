...
export const sampleMedicalData: MedicalFormData = {
  ...
  // Alergias
  hasAllergies: true,
  allergies: ["Penicilina", "Frutos do mar", "Pólen"],
  ...
  // Observações
  observations:
    "Paciente com histórico familiar de problemas cardíacos. Recomendado monitoramento regular da pressão arterial. Alergia severa à penicilina - utilizar alternativas como amoxicilina.",
};
...
export const sampleSubscriptions: Record<
  SubscriptionType,
  {
    name: string;
    price: number;
...
export const devUtils = {
  ...
  generateRandomTestData: (): Partial<MedicalFormData> => ({
    ...
    phone: `(11) ${Math.floor(Math.random() * 90000 + 10000)}-${Math.floor(
      Math.random() * 9000 + 1000
    )}`,
    birthDate: "1990-01-01",
    bloodType: ["A+", "A-", "B+", "B-", "AB+", "AB-", "O+", "O-"][
      Math.floor(Math.random() * 8)
    ] as "A+" | "A-" | "B+" | "B-" | "AB+" | "AB-" | "O+" | "O-",
