...
const ProfileDataSchema = z.object({
  ...
  emergencyContact: z.object({
    ...
    phone: z
      .string()
      .regex(/^\d{10,11}$/, "Telefone do contato deve conter 10 ou 11 dígitos"),
    relationship: z
      .string()
  ...
  static validateAndClean(data: unknown): PaymentWithProfileData {
    try {
      ...
      if (parsed.profileData.emergencyContact.phone) {
        parsed.profileData.emergencyContact.phone =
          parsed.profileData.emergencyContact.phone.replace(/\D/g, "");
      }
      ...
      const expectedAmount =
        expectedAmounts[parsed.profileData.subscriptionPlan];
      if (Math.abs(parsed.amount - expectedAmount) > 0.01) {
        throw new Error(
          `Valor incorreto para o plano ${parsed.profileData.subscriptionPlan}`
        );
      }
      ...
      if (error instanceof z.ZodError) {
        ...
        throw new Error(
          `Erro de validação: ${firstError.path.join(".")} - ${
            firstError.message
          }`
        );
      }
